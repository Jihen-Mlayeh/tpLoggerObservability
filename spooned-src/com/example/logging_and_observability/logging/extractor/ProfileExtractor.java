...
public class ProfileExtractor {
    ...
    public Map<String, UserProfile> extractProfilesFromLogs() {
        ...
        Map<String, List<LogEntry>> logsByUser = logs.stream().filter(log -> log.getUserEmail() != null).collect(Collectors.groupingBy(LogEntry::getUserEmail));
    ...
    private UserProfile buildProfileFromLogs(String userEmail, List<LogEntry> logs) {
        // Extract user info
        String userName = logs.stream().map(LogEntry::getUserName).filter(Objects::nonNull).findFirst().orElse("Unknown");
        // Count operations by type
        long readOps = logs.stream().filter(log -> "READ".equals(log.getOperationType())).count();
        long writeOps = logs.stream().filter(log -> "WRITE".equals(log.getOperationType())).count();
        long expensiveOps = logs.stream().filter(log -> "SEARCH_EXPENSIVE".equals(log.getOperationType())).count();
        ...
        profile.setProfileCreatedAt(logs.stream().map(LogEntry::getTimestamp).min(LocalDateTime::compareTo).orElse(LocalDateTime.now()));
        profile.setLastActivityAt(logs.stream().map(LogEntry::getTimestamp).max(LocalDateTime::compareTo).orElse(LocalDateTime.now()));
        // Convert LogEntry to OperationLog
        List<OperationLog> operationHistory = logs.stream().map(this::convertToOperationLog).collect(Collectors.toList());
    ...
    private ReadHeavyProfile buildReadHeavyProfile(String userName, String userEmail, List<LogEntry> logs) {
        ...
        profile.setGetAllProductsCount(((int) (logs.stream().filter(log -> "getAllProducts".equals(log.getAction())).count())));
        profile.setGetProductByIdCount(((int) (logs.stream().filter(log -> "getProductById".equals(log.getAction())).count())));
        // Track product views
        logs.stream().filter(log -> "getProductById".equals(log.getAction())).filter(log -> log.getResourceId() != null).forEach(log -> {
    ...
    private WriteHeavyProfile buildWriteHeavyProfile(String userName, String userEmail, List<LogEntry> logs) {
        ...
        profile.setTotalWriteOperations(((int) (writeOps)));
        // Count specific operations
        profile.setAddProductCount(((int) (logs.stream().filter(log -> "addProduct".equals(log.getAction())).count())));
        profile.setUpdateProductCount(((int) (logs.stream().filter(log -> "updateProduct".equals(log.getAction())).count())));
        profile.setDeleteProductCount(((int) (logs.stream().filter(log -> "deleteProduct".equals(log.getAction())).count())));
        // Track products modified
        logs.stream().filter(log -> "WRITE".equals(log.getOperationType())).filter(log -> log.getResourceId() != null).forEach(log -> {
    ...
    private ExpensiveProductSeekerProfile buildExpensiveSeekerProfile(String userName, String userEmail, List<LogEntry> logs) {
        ...
        logs.stream().filter(log -> log.getResourcePrice() != null).forEach(log -> {
            String productId = log.getResourceId();
            String productName = log.getResourceName();
    ...
    public void exportProfiles(Map<String, UserProfile> profiles) {
        ...
        for (Map.Entry<String, UserProfile> entry : profiles.entrySet()) {
            ...
            String fileName = String.format("%s/%s_%s_extracted.json", EXTRACTED_PROFILES_DIR, userEmail.replaceAll("[^a-zA-Z0-9]", "_"), profile.getProfileType());
            try {
        ...
        log.info("Profile extraction complete. {} profiles exported to {}", profiles.size(), EXTRACTED_PROFILES_DIR);
    }
    ...
    public String generateExtractionReport(Map<String, UserProfile> profiles) {
        ...
        report.append("╚════════════════════════════════════════════════════════╝\n\n");
        long readHeavy = profiles.values().stream().filter(p -> p instanceof ReadHeavyProfile).count();
        long writeHeavy = profiles.values().stream().filter(p -> p instanceof WriteHeavyProfile).count();
        long expensiveSeekers = profiles.values().stream().filter(p -> p instanceof ExpensiveProductSeekerProfile).count();
