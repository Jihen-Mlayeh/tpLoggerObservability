...
public class ExpensiveProductSeekerProfile extends UserProfile {
    ...
    public String getProfileDescription() {
        return String.format("User interested in expensive products (%.1f%% of views > €%.2f)", expensiveProductViewPercentage, EXPENSIVE_THRESHOLD);
    }
    ...
    public void trackProductView(String productId, String productName, Double price) {
        ...
        if ((price != null) && (price >= EXPENSIVE_THRESHOLD)) {
            ...
            ExpensiveProductView existing = expensiveProducts.stream().filter(p -> p.getProductId().equals(productId)).findFirst().orElse(null);
            if (existing != null) {
    ...
    private void updateStatistics(Double price) {
        ...
        // Calculate average
        DoubleSummaryStatistics stats = operationHistory.stream().filter(log -> log.getProductPrice() != null).mapToDouble(OperationLog::getProductPrice).summaryStatistics();
        if (stats.getCount() > 0) {
    ...
    public List<ExpensiveProductView> getTopExpensiveProducts(int limit) {
        return expensiveProducts.stream().sorted((a, b) -> Double.compare(b.getPrice(), a.getPrice())).limit(limit).toList();
    }
}