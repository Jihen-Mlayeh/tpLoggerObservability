...
public class UserProfileService {
     ...
     */
    public void logOperation(User user, String operationName, UserOperationType operationType, String productId, String productName, Double productPrice) {
        String userKey = getUserKey(user);
        UserProfile profile = userProfiles.computeIfAbsent(userKey, k -> createInitialProfile(user));
        // Create operation log
        OperationLog opLog = new OperationLog(operationName, operationType, LocalDateTime.now(), user.getName(), user.getEmail());
        ...
        if (!newProfile.getClass().equals(profile.getClass())) {
            log.info("Profile type changed for user {} from {} to {}", user.getName(), profile.getProfileType(), newProfile.getProfileType());
     ...
     */
    private void updateProfileStatistics(UserProfile profile, String operationName, UserOperationType operationType, String productId, String productName, Double productPrice) {
    ...
    private UserProfile determineProfileType(UserProfile currentProfile, User user) {
        ...
        long readOps = currentProfile.getOperationHistory().stream().filter(log -> log.getOperationType() == UserOperationType.READ).count();
        long writeOps = currentProfile.getOperationHistory().stream().filter(log -> log.getOperationType() == UserOperationType.WRITE).count();
        long expensiveViews = currentProfile.getOperationHistory().stream().filter(log -> (log.getProductPrice() != null) && (log.getProductPrice() >= 100.0)).count();
    ...
    private void reprocessOperations(UserProfile profile) {
        for (OperationLog log : profile.getOperationHistory()) {
            updateProfileStatistics(profile, log.getOperationName(), log.getOperationType(), log.getProductId(), log.getProductName(), log.getProductPrice());
        }
    }
    ...
    public void exportProfileToJson(User user) throws IOException {
        ...
        String fileName = String.format("%s/%s_%s_profile.json", PROFILES_DIRECTORY, user.getEmail().replaceAll("[^a-zA-Z0-9]", "_"), profile.getProfileType());
    ...
    public void exportAllProfiles() {
        userProfiles.forEach((key, profile) -> {
            try {
                String fileName = String.format("%s/%s_%s_profile.json", PROFILES_DIRECTORY, profile.getUserEmail().replaceAll("[^a-zA-Z0-9]", "_"), profile.getProfileType());
                objectMapper.writeValue(new File(fileName), profile);
    ...
    public String generateSummaryReport() {
        ...
        report.append("╚════════════════════════════════════════╝\n\n");
        long readHeavy = userProfiles.values().stream().filter(p -> p instanceof ReadHeavyProfile).count();
        long writeHeavy = userProfiles.values().stream().filter(p -> p instanceof WriteHeavyProfile).count();
        long expensiveSeekers = userProfiles.values().stream().filter(p -> p instanceof ExpensiveProductSeekerProfile).count();
