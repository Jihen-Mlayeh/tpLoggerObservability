...
public class OpenTelemetryInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        Context extractedContext = openTelemetry.getPropagators().getTextMapPropagator().extract(Context.current(), request, new HttpServletRequestGetter());
        // Créer un span pour cette requête
        Span span = tracer.spanBuilder((request.getMethod() + " ") + request.getRequestURI()).setParent(extractedContext).startSpan();
