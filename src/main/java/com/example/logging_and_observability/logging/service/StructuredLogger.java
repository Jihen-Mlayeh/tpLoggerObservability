...
public class StructuredLogger {
     ...
     */
    public void logProductOperation(User user, String action, Product product,
                                    String operationType, long duration) {
        LogEntry.Builder builder = LogEntry.builder()
                .timestamp(LocalDateTime.now())
                .level("INFO")
                .logger("ProductService")
                .event("PRODUCT_OPERATION")
                .action(action)
        ...
        if (product != null) {
            builder.resourceId(product.getId())
                    .resourceName(product.getName())
                    .resourcePrice(product.getPrice());
        }
    ...
    public void logExpensiveProductView(User user, Product product) {
        LogEntry entry = LogEntry.builder()
                ...
                .level("INFO")
                .logger("ProductService")
                .event("EXPENSIVE_PRODUCT_VIEW")
                .userName(user.getName())
                .userEmail(user.getEmail())
                .action("viewExpensiveProduct")
                .operationType("SEARCH_EXPENSIVE")
                .resourceType("PRODUCT")
                .resourceId(product.getId())
                .resourceName(product.getName())
                .resourcePrice(product.getPrice())
                .result("SUCCESS")
                .additionalInfo("Price >= €100")
                .build();
    ...
    public void logProfileGeneration(User user, String profileType) {
        LogEntry entry = LogEntry.builder()
                ...
                .result("SUCCESS")
                .additionalInfo("Profile Type: " + profileType)
                .build();
    ...
    private void writeLog(LogEntry entry) {
        ...
        if (logBuffer.size() % 10 == 0) { // Batch write every 10 entries
            writeToJsonFile();
        }
    ...
    public List<LogEntry> getLogsByUser(String userEmail) {
        return logBuffer.stream()
                .filter(log -> userEmail.equals(log.getUserEmail()))
                .toList();
    }
