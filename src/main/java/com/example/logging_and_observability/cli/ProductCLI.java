...
public class ProductCLI implements CommandLineRunner {
        ...
        if (expensiveProducts.isEmpty()) {
            ...
            for (Product p : expensiveProducts) {
                ...
                userProfileService.logOperation(
                        ...
                        com.example.logging_and_observability.profiling.model.UserOperationType.SEARCH_EXPENSIVE,
                        p.getId(),
                        p.getName(),
                        p.getPrice()
                );
    ...
    private void displayUserProfile() {
        ...
        userProfileService.getUserProfile(currentUser).ifPresentOrElse(
                profile -> {
                    System.out.println("\n╔════════════════════════════════════════╗");
                    ...
                    } else if (profile instanceof com.example.logging_and_observability.profiling.model.WriteHeavyProfile writeProfile) {
                    ...
                    } else if (profile instanceof com.example.logging_and_observability.profiling.model.ExpensiveProductSeekerProfile expProfile) {
    ...
    private void exportUserProfile() {
        ...
        try {
            ...
            System.out.println("📁 Location: user-profiles/" +
                    currentUser.getEmail().replaceAll("[^a-zA-Z0-9]", "_") + "_*_profile.json");
        } catch (IOException e) {
